...
impl SystemTickTimer {
    ...
    pub fn enable(cpu_clock_frequency_in_hertz: u32) {
    ...
    ///
    fn delay_for_1_ms(cpu_clock_frequency_in_hertz: u32) {
        let stk_value_write_ptr = STK_VAL as *mut u32;
        let stk_ctrl_read_ptr = STK_CTRL as *const u32;
        unsafe {
            // ptr::write_volatile(stk_value_write_ptr, 0x00);

            // hprintln!("start to wait ---->");
            while (ptr::read_volatile(stk_ctrl_read_ptr) & STK_CTRL_COUNTDOWN_TO_ZERO_BIT)
                >> STK_CTRL_COUNTDOWN_TO_ZERO_START_BIT
                != 1
            {
                asm::nop();
                // hprintln!("value: {} ---->", ptr::read_volatile(stk_ctrl_read_ptr));
            }
            // hprintln!("end to wait ---->");
        }
    }

    ///
    pub fn delay_for_ms(cpu_clock_frequency_in_hertz: u32, milliseconds_to_wait: u32) {
        let stk_ctrl_write_ptr = STK_CTRL as *mut u32;
        let stk_ctrl_read_ptr = STK_CTRL as *const u32;

        let stk_reload_write_ptr = STK_LOAD as *mut u32;
        let stk_reload_read_ptr = STK_LOAD as *const u32;
        ...
        while delay_count < milliseconds_to_wait {
            let mut current_reload = unsafe { ptr::read_volatile(stk_reload_read_ptr) };
            let mut current_countdown = unsafe { ptr::read_volatile(stk_val_read_ptr) };
            let mut ctrl_register_value = unsafe { ptr::read_volatile(stk_ctrl_read_ptr) };
            let mut counted_to_zero = (ctrl_register_value & STK_CTRL_COUNTDOWN_TO_ZERO_BIT)
                >> STK_CTRL_COUNTDOWN_TO_ZERO_START_BIT;
            // while (unsafe { ptr::read_volatile(stk_ctrl_read_ptr) }
            // & STK_CTRL_COUNTDOWN_TO_ZERO_BIT)
            // >> STK_CTRL_COUNTDOWN_TO_ZERO_START_BIT
            // != 1
            // {
            while counted_to_zero != 1 {
                // current_countdown = unsafe { ptr::read_volatile(stk_val_read_ptr) };
                // hprintln!("current_countdown: {} ---->", current_countdown);
                // asm::nop();
            }
        ...
        // Disable
        unsafe {
            ptr::write_volatile(stk_ctrl_write_ptr as *mut u32, 0x00000000);
        }
    ...
    ///
    pub fn delay(cpu_clock_frequency_in_hertz: u32, milliseconds_to_wait: u32) {
        for _ in 1..milliseconds_to_wait {
            Self::delay_for_1_ms(cpu_clock_frequency_in_hertz);
        }
    }
